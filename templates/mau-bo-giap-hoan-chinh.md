...
> - **Quy cách (Chuẩn 2026-09-08)**: 4 món trọn vẹn: Mũ (head), Áo giáp (body), Quần (legs), Giày (feet).
> - **Điểm nhấn**: Cẳng tay gộp vào Áo giáp (`slot === 'forearm'`), Giày (`feet`) có hàm riêng biệt.
...
// Gồm Thân ngực (torso) + Ống bắp tay (upperArm) + Ống cẳng tay (forearm)
// ─────────────────────────────────────────────────────────────────────────────
export function body(w, h, d, pal, slot) {
  const mats = M(pal);

  // Ống bắp tay: gắn vào mesh bắp tay -> vung cùng vai khi đánh/chạy
  if (slot === 'upperArm') {
    const b = piece(mats);
  ...
  // Ống cẳng tay: gắn vào mesh cẳng tay -> vung cùng khuỷu tay
  if (slot === 'forearm') {
  ...
  // Cầu vai (Pauldron vòm sâu che trùm đỉnh bắp tay)
  b.both((sx) => {
...
// ─────────────────────────────────────────────────────────────────────────────
// 3. QUẦN THIẾT GIÁP (Legs)
// Phủ trọn 2 đoạn chân: Đùi (thigh) & Cẳng chân (shin)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// 4. GIÀY THIẾT GIÁP (Feet)
// Bàn chân riêng biệt (anchor 'foot') — Không còn cần mẹo đoán tỷ lệ isFoot
// ─────────────────────────────────────────────────────────────────────────────
...
// Export đủ 4 món theo chuẩn kiến trúc 2026-09-08:
